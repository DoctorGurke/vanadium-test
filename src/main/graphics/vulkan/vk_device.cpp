...
VulkanDevice::VulkanDevice(VkPhysicalDevice physicalDevice)
{
    ...
    // Memory properties are used for creating buffers
...
uint32_t VulkanDevice::getQueueFamilyIndex(VkQueueFlags queueFlags) const
{
    ...
    {
        for (uint32_t i = 0; i < static_cast<uint32_t>(queueFamilyProperties
                                                           .size());
             i++)
        {
        ...
        for (const char *enabledExtension : deviceExtensions)
        {
            if (!extensionSupported(enabledExtension))
            {
                Log::Error(std::format(
                    "Enabled device extension {0} is not present at device level!",
                    enabledExtension));
            }
        }
                                    ...
                                    void *data)
{
    ...
    Debug::CheckVulkan(vkCreateBuffer(logicalDevice,
                                      &bufferCreateInfo,
                                      nullptr,
                                      buffer));
    ...
    memAlloc.memoryTypeIndex = getMemoryType(
                                    ...
                                    void *data)
{
    ...
    Debug::CheckVulkan(vkAllocateMemory(logicalDevice,
                                        &memAlloc,
                                        nullptr,
                                        &buffer->memory));
 ...
 */
void VulkanDevice::copyBuffer(Buffer *src,
                              Buffer *dst,
                              VkQueue queue,
    ...
    VkCommandPoolCreateFlags createFlags)
{
    ...
    Debug::CheckVulkan(
        vkCreateCommandPool(logicalDevice,
                            &cmdPoolInfo,
                            nullptr,
                            &cmdPool));
    return cmdPool;
    ...
    bool begin)
{
    ...
    if (begin)
    {
        ...
        Debug::CheckVulkan(vkBeginCommandBuffer(cmdBuffer, &cmdBufInfo));
    }
    return cmdBuffer;
}
                                      ...
                                      bool free)
{
    ...
    Debug::CheckVulkan(vkEndCommandBuffer(commandBuffer));
    ...
    // Submit to the queue
    Debug::CheckVulkan(vkQueueSubmit(queue, 1, &submitInfo, fence));
    // Wait for the fence to signal that command buffer has finished executing
    Debug::CheckVulkan(vkWaitForFences(logicalDevice,
                                       1,
                                       &fence,
                                       VK_TRUE,
